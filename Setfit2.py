...
def create_features(data, feature_columns, cache_prefix='', cache_dir='embedding_cache'):
    print(f"Creating features for {cache_prefix} using SentenceTransformer")
    model = SentenceTransformer('all-MiniLM-L6-v2')
    
    os.makedirs(cache_dir, exist_ok=True)
    cache_path = os.path.join(cache_dir, f"{cache_prefix}_embeddings.pkl")

    if os.path.exists(cache_path):
        print(f"Loading cached {cache_prefix} embeddings")
        ...
        print(f"Caching {cache_prefix} embeddings")
...
def predict_unseen_data(model, id_to_label, unseen_data, feature_columns):
    ...
    def create_sentence_from_features(row):
        parts = []
        for col in feature_columns:
            feature_value = row[col]
            if pd.notna(feature_value) and feature_value != '':
    ...
    unseen_texts = unseen_data.apply(lambda row: create_sentence_from_features(row, feature_columns), axis=1).tolist()
